<div class="cta catTop">
    <section class="secCatTop">
        <h2 class="t_center cl_5F161E fw_700 h2CatTop">まずは無料相談</h2>
        <p class="t_center cl_5F161E fw_700 txtCatTop">１級建築士がお話を伺います</p>
        <figure class="pointCatTop">
            <img loading="lazy" src="img/pointCatTop.svg" alt="お問合せ/ご相談/現地調査/お見積" width="312" height="72">
        </figure>
        <div class="btnCtaLxn">
            <a class="d_flex j_center ali_center bg_gra cl_fff fw_700 btnCta" href="https://houselabo-n.co.jp/materials/" target="_blank">資料請求・ご相談</a>
        </div>
    </section>
</div>